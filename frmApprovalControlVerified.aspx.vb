...
Partial Class frmApprovalControlVerified
	...
	Protected Sub AddViewIACComment_Click(sender As Object, e As EventArgs) Handles btnShowIACCommentPopup.Click
		...
		Me.txtIACApplicationID.Text = Me.gridApplications.Rows(i.RowIndex).Cells(4).Text
		Me.txtApplicationIACComment.Text = cr.PMgetApplicationComment(Me.gridApplications.Rows(i.RowIndex).Cells(4).Text, "PRE_IC").Rows(0).Item("txtComment").ToString
	...
	End Sub
     ...
     Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
		...
		Dim scriptManagerA As New ScriptManager, scriptManagerb As New ScriptManager, scriptManagerC As New ScriptManager
		...
		scriptManagerb.RegisterPostBackControl(Me.btnSchedule)


		scriptManagerC = ScriptManager.GetCurrent(Me.Page)
		scriptManagerC.RegisterPostBackControl(Me.imgDownloadSoft)


          'Dim roleName As String, accesslevel As Integer
          'Session("accesslevel") = 2
          'Session("roleName") = "IC"

          'If IsNothing(Session("accesslevel")) = False Then
          '     accesslevel = CInt(Session("accesslevel"))
          'Else
          '     accesslevel = 0
          'End If

          'If IsNothing(Session("roleName")) = False Then
          '     roleName = CStr(Session("roleName"))
          'Else
          '     roleName = ""
          'End If
          ...
          '     Me.dvChecked.Visible = False
          '     Me.dvAuthorised.Visible = True
          '     Me.dvVerified.Visible = False

          'ElseIf roleName = "IC" And accesslevel = 1 Then

          '     Me.dvChecked.Visible = True
          '     Me.dvAuthorised.Visible = False
          '     Me.dvVerified.Visible = False

          'ElseIf roleName = "IC" And accesslevel = 2 Then
	...
	End Sub
     ...
     Protected Sub btnFind_Click(sender As Object, e As EventArgs) Handles btnFind.Click
		...
		dt = cr.PMgetPencomApprovalBatchByType(apptypeID, Me.ddExportedBatches.SelectedItem.Text, True, "V")
	...
	End Function
     ...
     Protected Sub btnTagAll_Click(sender As Object, e As EventArgs) Handles btnTagAll.Click
          ...
          For Each grow As GridViewRow In Me.gridApplications.Rows
               ...
               Dim cb As CheckBox = TryCast(grow.Cells(0).FindControl("ChkPINApprovalVerified"), CheckBox)
     ...
     '               cb = grow.FindControl("ChkPINApprovalChecked")

     '               If cb.Checked = True And cb.Enabled = True Then
     '                    PMUpdateApprovalControlCheck(grow.Cells(4).Text, "o-taiwo", "CHECKED")
     '               Else
     '               End If
     ...
     Public Sub PMUpdateApprovalControlCheck(appCode As String, uName As String, stage As String)
          Try
               ...
               If stage = "CHECKED" Then
				...
				myComm.CommandText = "update tblApplicationApprovalPayee set  txtControlCheckedBy = '" & uName & "',dteChecked = '" & DateTime.Parse(Now).ToString("yyyy-MM-dd HH:MM:SS") & "' where txtApplicationCode = '" & appCode & "'"
                    command.CommandType = CommandType.Text
                    myComm.ExecuteNonQuery()
               ...
               ElseIf stage = "Authorize" Then
				...
				myComm.CommandText = "update tblApplicationApprovalPayee set  txtControlAuthorisedBy = '" & uName & "',dteAuthorised = '" & DateTime.Parse(Now).ToString("yyyy-MM-dd HH:MM:SS") & "' where txtApplicationCode = '" & appCode & "'"
                    command.CommandType = CommandType.Text
                    myComm.ExecuteNonQuery()
     ...
     Protected Sub btnVerified_Click(sender As Object, e As EventArgs) Handles btnVerified.Click
          ...
          Try
               ...
               Else
                    For Each grow As GridViewRow In Me.gridApplications.Rows
                         ...
                         cb = grow.FindControl("ChkPINApprovalVerified")

                         If cb.Checked = True And cb.Enabled = True Then
                              PMUpdateApprovalControlCheck(grow.Cells(4).Text, CStr(Session("user")), "VERIFIED")
                         Else
                         End If
     ...
     Protected Sub btnSchedule_Click(sender As Object, e As EventArgs) Handles btnSchedule.Click
		...
		Try
			...
			'generateFiles(Me.ddExportedBatches.SelectedItem.Text, filePath)
			generateFiles(Me.ddExportedBatches.SelectedItem.Text, filePath, apptypeID)
	...
	Private Sub generateFiles(approvalBatchNum As String, path As String, apptypeID As Integer)
		...
		'   MsgBox("" & ds.Tables(0).Rows(0).Item("txtConfirmedBy").ToString())
	...
	Private Function populateSchedule(approvalBatchNum As String, apptype As Integer) As DataSet
		...
		dtApplicationSchedule = cr.PMgetApprovalPaymentSchedule(approvalBatchNum, apptype, "A")
	...
	End Function
     ...
     Protected Sub btnCancelVerified_Click(sender As Object, e As EventArgs) Handles btnCancelVerified.Click
          Try
			...
			Else
				...
				cr.PMCancelPaymentControls(CStr(ViewState("AppCode")), Session("user"), "VERIFIED")
				Dim apptypeID As Integer = getApprovalType(Me.ddApplicationType.SelectedValue)
				dt = cr.PMgetPencomApprovalBatchByType(apptypeID, Me.ddExportedBatches.SelectedItem.Text, True)
				ViewState("BatchApprovals") = dt
				BindGrid(dt)
	...
	Protected Sub btnReject_Click(sender As Object, e As EventArgs) Handles btnReject.Click
		...
		Else
			For Each grow As GridViewRow In Me.gridApplications.Rows
				...
				If cb.Checked = True And cb.Enabled = True Then
						'PMUpdateApprovalControlCheck(grow.Cells(4).Text, CStr(Session("user")), "VERIFIED")
						cr.PMApprovalPayeeStatus(grow.Cells(4).Text, Session("user"), "P", 0)
				Else
				End If
