...
Partial Class frmVCCalculatorNew
	...
	Protected Function getCurrentValueDiscounted(numAmount As Double, freq As Integer) As Double
		Dim curValDiscounted As Double

		Select Case freq

			Case freq = 1 ' weekly
				curValDiscounted = 0.7 * 52 * numAmount
			Case freq = 2 ' bi-weekly
				curValDiscounted = 0.7 * 26 * numAmount
			Case freq = 3 ' monthly
				curValDiscounted = 0.7 * 12 * numAmount
			Case Else
				' annually
				curValDiscounted = 0.7 * numAmount

		End Select

		Return curValDiscounted
	End Function
	...
	Protected Sub btnFind_Click(sender As Object, e As EventArgs) Handles btnFind.Click
		...
		Try
			...
			If CDbl(Me.txtRSABalance.Text) > 0 Then
				...
				txtCurRSAValue.Text = CDbl(Me.txtRSABalance.Text)
			...
			Else
				...
				txtCurRSAValue.Text = FormatNumber(CDbl(Me.txtRSABalance.Text), 2)
				'txtFV.Text = 0
				ViewState("numRSABalance") = numRSABalance
			End If
			...
			'field1
			txtFVDOB.Text = dt.Rows(0).Item("dateofbirth")

			'field2
			txtYearsToRetirement.Text = FormatNumber(CInt(txtFVAge.Text) - ((DateDiff(DateInterval.Day, CDate(txtFVDOB.Text), Now.Date)) / 365), 6)
			'field3
			txtMonthsToRetirement.Text = FormatNumber(txtYearsToRetirement.Text * CInt(txtPeriodicity.Text), 6)

			'C12 = FormatNumber(CDbl(Me.txtNominalAnnual.Text.Split("%")(0)) / 100, 5)

			'field4
			txtNominalMonthly.Text = (1 + (FormatNumber(CDbl(Me.txtNominalAnnual.Text.Split("%")(0)) / 100, 5))) ^ (1 / Me.txtPeriodicity.Text) - 1
			'field5
			txtContGrowthRate.Text = (1 + (FormatNumber(CDbl(Me.txtSalaryGrowthAnnual.Text.Split("%")(0)) / 100, 5))) ^ (1 / Me.txtPeriodicity.Text) - 1

			'field6
			txtFVGrowthAsset.Text = FormatNumber((Me.txtMonthlyCont.Text / (txtNominalMonthly.Text - CDbl(txtContGrowthRate.Text))) * ((1 + txtNominalMonthly.Text) ^ (txtMonthsToRetirement.Text) - (1 + CDbl(txtContGrowthRate.Text)) ^ txtMonthsToRetirement.Text), 2)

			'field7
			txtFVCurAsset.Text = FormatNumber(txtCurRSAValue.Text * (1 + FormatNumber(CDbl(Me.txtNominalAnnual.Text.Split("%")(0)) / 100, 5)) ^ Me.txtYearsToRetirement.Text, 2)

			'field8
			txtFVContribution.Text = FormatNumber(CDbl(txtFVGrowthAsset.Text) + CDbl(txtFVCurAsset.Text), 2)
			...
			MsgBox("" & txtRecommendedLumpSum.Text)
			...
			'finalField9
			txtFV.Text = FormatNumber(CDbl(((CDbl(txtRSABalance.Text) * CDbl(txtProjectLifeStyle.Text)) / CDbl(txtMaxMonthlyDraw.Text))), 2)


			Dim pmt As Double


			pmt = wsf.Pmt(txtNominalMonthly.Text, Me.txtMonthsToRetirement.Text, 0, -1 * (txtFVContribution.Text - txtFV.Text), 0)

			If pmt < 0 Then

			Else
				lblMessage.InnerText = "Sure, You Are Good with " & FormatNumber(pmt, 2) & " : " & FormatNumber(((pmt / txtMonthlyCont.Text) * 100), 2).ToString & "% of Your Current Monthly Contribution"
			End If
	...
	Protected Sub btnProjectedLifeStyle_Click(sender As Object, e As EventArgs) Handles btnProjectedLifeStyle.Click

		Dim numProjectedLifeStyle As Double = 0

		'numProjectedLifeStyle = numProjectedLifeStyle + getCurrentValueDiscounted(CDbl(Me.txtFood.Text), 4)
		'numProjectedLifeStyle = numProjectedLifeStyle + getCurrentValueDiscounted(CDbl(Me.txtHousing.Text), 4)
		'numProjectedLifeStyle = numProjectedLifeStyle + getCurrentValueDiscounted(CDbl(Me.txtTransportation.Text), 4)
		'numProjectedLifeStyle = numProjectedLifeStyle + getCurrentValueDiscounted(CDbl(Me.txtHealth.Text), 4)
		'numProjectedLifeStyle = numProjectedLifeStyle + getCurrentValueDiscounted(CDbl(Me.txtFinancial.Text), 4)
		'numProjectedLifeStyle = numProjectedLifeStyle + getCurrentValueDiscounted(CDbl(Me.txtOthers.Text), 4)

		txtProjectLifeStyle.Text = numProjectedLifeStyle

		Dim excel = New Microsoft.Office.Interop.Excel.Application()
		Dim wsf As Microsoft.Office.Interop.Excel.WorksheetFunction = excel.WorksheetFunction

		numProjectedLifeStyle = numProjectedLifeStyle + wsf.Fv(FormatNumber(CDbl("7.58%".ToString.Split("%")(0)) / 100, 5), CDbl(Me.txtYearsToRetirement.Text), 0, -1 * getCurrentValueDiscounted(CDbl(Me.txtFood.Text), 4), 0)

		numProjectedLifeStyle = numProjectedLifeStyle + wsf.Fv(FormatNumber(CDbl("7.58%".ToString.Split("%")(0)) / 100, 5), CDbl(Me.txtYearsToRetirement.Text), 0, -1 * getCurrentValueDiscounted(CDbl(Me.txtHousing.Text), 4), 0)

		numProjectedLifeStyle = numProjectedLifeStyle + wsf.Fv(FormatNumber(CDbl("7.58%".ToString.Split("%")(0)) / 100, 5), CDbl(Me.txtYearsToRetirement.Text), 0, -1 * getCurrentValueDiscounted(CDbl(Me.txtTransportation.Text), 4), 0)

		numProjectedLifeStyle = numProjectedLifeStyle + wsf.Fv(FormatNumber(CDbl("7.58%".ToString.Split("%")(0)) / 100, 5), CDbl(Me.txtYearsToRetirement.Text), 0, -1 * getCurrentValueDiscounted(CDbl(Me.txtHealth.Text), 4), 0)

		numProjectedLifeStyle = numProjectedLifeStyle + wsf.Fv(FormatNumber(CDbl("7.58%".ToString.Split("%")(0)) / 100, 5), CDbl(Me.txtYearsToRetirement.Text), 0, -1 * getCurrentValueDiscounted(CDbl(Me.txtFinancial.Text), 4), 0)

		numProjectedLifeStyle = numProjectedLifeStyle + wsf.Fv(FormatNumber(CDbl("7.58%".ToString.Split("%")(0)) / 100, 5), CDbl(Me.txtYearsToRetirement.Text), 0, -1 * getCurrentValueDiscounted(CDbl(Me.txtOthers.Text), 4), 0)

		txtProjectLifeStyle.Text = numProjectedLifeStyle / 12

		If txtProjectLifeStyle.Text = "0" Then
			txtProjectLifeStyle.Text = ""
		Else
	...
	Protected Sub ddLifeStyle_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddLifeStyle.SelectedIndexChanged

		Select Case Me.ddLifeStyle.Text

			Case Is = "Premium"
				txtProjectLifeStyle.Text = "3345805.86"
				txtFood.Enabled = False
				txtHousing.Enabled = False
				txtTransportation.Enabled = False
				txtHealth.Enabled = False
				txtFinancial.Enabled = False
				txtOthers.Enabled = False
			Case Is = "Classic"
				txtProjectLifeStyle.Text = "2230537.24"
				txtFood.Enabled = False
				txtHousing.Enabled = False
				txtTransportation.Enabled = False
				txtHealth.Enabled = False
				...
				txtOthers.Enabled = False
			Case Is = "Basic"
				txtProjectLifeStyle.Text = "836451.47"
				txtFood.Enabled = False
				txtHousing.Enabled = False
				txtTransportation.Enabled = False
				txtHealth.Enabled = False
				...
				txtOthers.Enabled = False
			Case Else
				txtProjectLifeStyle.Text = 0
				txtFood.Enabled = True
				txtHousing.Enabled = True
				txtTransportation.Enabled = True
				txtHealth.Enabled = True
				txtFinancial.Enabled = True
