...
Partial Class frmStandingIRun
	...
	Protected Sub generateSI()
		...
		If ddRunType.Text = "Renewal" Then
			Dim SPO As New StandingPaymentOrder
			SPO.RunBy = Session("user")
			SPO.MonthFor = intAnnMonth
			SPO.YearFor = CInt(ddAnnYear.Text)
			cr.PMRenewalStandingInstruction(SPO)

			dt = cr.PMgetNewStandingOrder("R", "P", intAnnMonth, CInt(ddAnnYear.Text))

			ViewState("Renewal") = dt

			If dt.Rows.Count = 0 Then

				dt = cr.PMgetNewStandingOrder("R", "AA", intAnnMonth, CInt(ddAnnYear.Text))

				If dt.Rows.Count > 0 Then

					dvExportButton.Visible = True
					dvExportMessage.Visible = True
					Me.btnVet.Enabled = False

				Else

					dvExportButton.Visible = False
					dvExportMessage.Visible = False
		...
		Else
			...
			cr.PMgenerateStandingInstruction(Session("user"), intAnnMonth, CInt(ddAnnYear.Text), "N")
			dt = cr.PMgetNewStandingOrder("N", "P", intAnnMonth, CInt(ddAnnYear.Text))

			ViewState("New") = dt

			If dt.Rows.Count = 0 Then
				dt = cr.PMgetNewStandingOrder("N", "AA", intAnnMonth, CInt(ddAnnYear.Text))
	...
	Protected Sub gridSIRun_RowDataBound(sender As Object, e As GridViewRowEventArgs)
		...
		If IsNothing(ViewState("dt")) = False Then
			...
			If e.Row.RowType = DataControlRowType.DataRow Then
				...
				If (dt.Rows(e.Row.RowIndex).Item("txtStatus")) = "AA" Then

					Dim cbChecked As CheckBox = TryCast(e.Row.FindControl("ChkPensionerChecked"), CheckBox)
	...
	Private Sub generateFiles(SIApprovalID As Integer, filePath As String)
	...
	Protected Sub BtnViewSI_Click(sender As Object, e As EventArgs)
		...
		'Me.gridStandingOrder.Rows(j.RowIndex).it()
	...
	'generating a dataset object to be used to populate the report with data
	Protected Function populateSI(SIApprovalID As Integer) As DataSet
		...
		Try
			...
			Dim MonthTypeCollection As New Hashtable
			' getting a customer's standing order already generated
			...
			MonthTypeCollection.Add("1", "Jan")
			MonthTypeCollection.Add("2", "Feb")
			MonthTypeCollection.Add("3", "Mar")
			MonthTypeCollection.Add("4", "Apr")
			MonthTypeCollection.Add("5", "May")
			MonthTypeCollection.Add("6", "Jun")
			MonthTypeCollection.Add("7", "Jul")
			MonthTypeCollection.Add("8", "Aug")
			MonthTypeCollection.Add("9", "Sep")
			MonthTypeCollection.Add("10", "Oct")
			MonthTypeCollection.Add("11", "Nov")
			MonthTypeCollection.Add("12", "Dec")
			...
			Do While i < dtSI.Rows.Count
				...
				newSNRow("txtBank") = UCase(dtSI.Rows(i).Item("BankName").ToString) & " , " & UCase(dtSI.Rows(i).Item("BankBranch").ToString)
				...
				If dtSI.Rows(i).Item("txtVettedby").ToString <> "" Then
					'retrieving the scanned signature for the person that vetted the standing order
				...
				If dtSI.Rows(i).Item("txtChecker1").ToString <> "" Then
					'retrieving the scanned signature for the first control person that checked  the standing order
				...
				If dtSI.Rows(i).Item("txtChecker2").ToString <> "" Then
					'retrieving the scanned signature for the second control person that checked  the standing order
				...
				If dtSI.Rows(i).Item("txtApproval1").ToString <> "" Then
					'retrieving the scanned signature for the first signatory person that approved  the standing order
				...
				If dtSI.Rows(i).Item("txtApproval2").ToString <> "" Then
					'retrieving the scanned signature for the second signatory person that approved  the standing order
	...
	Protected Sub btnVet_Click(sender As Object, e As EventArgs) Handles btnVet.Click
			...
			If IsNothing(ViewState("TagAll")) = True Then
				...
				For Each grow As GridViewRow In Me.gridStandingOrder.Rows
					...
					If cb.Checked = True Then
						cr.PMUpdateStandingOrderApproval(CInt(grow.Cells(1).Text), Session("user"), "V")
					End If
				Next
				...
				If IsNothing(ViewState("New")) = False And IsNothing(ViewState("TagAll")) = False Then
					...
					If i = 1 Then
						...
						Do While i < dt.Rows.Count
							cr.PMUpdateStandingOrderApproval(dt.Rows(i).Item("pkiSIApproval"), Session("user"), "V")
							i = i + 1
						Loop
	...
	Protected Sub btnExportSIs_Click(sender As Object, e As EventArgs) Handles btnExportSIs.Click
		Dim dt As New DataTable, i As Integer, filePath As String
		Try

			If Directory.Exists("\\P-MIDAS2\NPM_Data\SI\") = True Then
			Else
				Directory.CreateDirectory("\\P-MIDAS2\NPM_Data\SI\")
			End If
			...
			Do While i < dt.Rows.Count
				...
				filePath = "\\P-MIDAS2\NPM_Data\SI\" & dt.Rows(i).Item("txtPIN").ToString & ".pdf"
				generateFiles(CInt(dt.Rows(i).Item("pkiSIApproval")), filePath)
				i = i + 1

			Loop
			spRowCount.InnerText = i & " Standing Order(s) Exported Successfully !"
			dvSPRowCount.Visible = True
	...
	Protected Sub chkViewApproved_CheckedChanged(sender As Object, e As EventArgs) Handles chkViewApproved.CheckedChanged

		If Me.chkViewApproved.Checked = True Then
	...
	Protected Sub btnViewButton_Click(sender As Object, e As EventArgs) Handles btnViewButton.Click
