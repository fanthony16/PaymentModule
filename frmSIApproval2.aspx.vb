...
Partial Class frmSIApproval2
	...
	Protected Sub generateSI()
		...
		If ddRunType.Text = "Renewal" Then
			...
			dt = cr.PMgetNewStandingOrder("R", "A", intAnnMonth, CInt(ddAnnYear.Text))
			ViewState("Renewal") = dt
		Else

			dt = cr.PMgetNewStandingOrder("N", "A", intAnnMonth, CInt(ddAnnYear.Text))
			ViewState("New") = dt
		End If
			...
			If IsNothing(ViewState("TagAll")) = True Then
				...
				For Each grow As GridViewRow In Me.gridStandingOrder.Rows
					...
					If cb.Checked = True Then
						cr.PMUpdateStandingOrderApproval(CInt(grow.Cells(1).Text), Session("user"), "AA")
					End If
				...
				If IsNothing(ViewState("New")) = False And IsNothing(ViewState("TagAll")) = False Then
					...
					If i = 1 Then
						...
						Do While i < dt.Rows.Count
							cr.PMUpdateStandingOrderApproval(dt.Rows(i).Item("pkiSIApproval"), Session("user"), "AA")
							i = i + 1
						Loop
