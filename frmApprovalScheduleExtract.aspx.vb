...
Partial Class frmApprovalScheduleExtract
	...
	Protected Sub btnViewTransaction_Click(sender As Object, e As EventArgs) Handles btnViewTransaction.Click
		Try
			...
			Dim apptypeID As Integer = getApprovalType(Me.ddApplicationType.SelectedValue), cr As New Core, returnType As Boolean

			If Me.chkRecallBatch.Checked = True Then

				returnType = cr.PMResetExtractedBatch(LTrim(RTrim(Me.txtPencomBatch.Text)), Server.MapPath("~/Logs"))
				If returnType = True Then
					spRecall.Visible = True
				ElseIf returnType = False Then
					spRecall.Visible = True
					spRecall.InnerText = "ReSet Failed"
				End If
