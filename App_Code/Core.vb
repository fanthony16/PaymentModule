...
Public Class Core
	...
	Public Sub PMUpdateApplication(AppDetail As ApplicationDetail, AppDocNew As List(Of ApplicationDocumentDetail), AppDocOLD As List(Of ApplicationDocumentDetail), AppAdhocDoc As List(Of AdhocDocuments), userName As String, AppCheckList As ApplicationCheckList, AppCheckListDBA As ApplicationCheckListDOB, logPath As String)
		...
		Catch ex As Exception
			...
			loger.Logger(ex.Message & "Core - Edit Application Submission")
			...
			If rtype = "F" Then
				...
				Select Case typeID
					...
					Case Is = 2
						...
						MyDataAdapter = New SqlClient.SqlDataAdapter("select a.txtApplicationCode,txtPIN,	replace(txtFullName,'|','') txtFullName,	txtEmployerName, dteApplicationDate,(select dteValueDate from tblApplicationApprovalPayee where txtApplicationCode = a.txtApplicationCode ) as ValueDate,isnull(b.txtstatus,'P') as txtstatus,b.numApproved, isnull(b.numPayingAmount,0) numPayingAmount,isnull(c.numAmountToPay,c.[twentyfive-percent-rsa-balance]) as ApprovedAmount,	isnull(c.numAmountToPay,c.[twentyfive-percent-rsa-balance]) AmountToPay,a.IsControlChecked, a.txtControlCheckedBy from dbo.tblMemberApplication a, tblApplicationApprovalPayee b,awbr500 c where a.txtApplicationCode = b.txtApplicationCode and a.txtApplicationCode = c.txtApplicationCode and isnull(b.txtstatus,'P') = 'F' and a.txtPencomBatch = @batchNo order by b.pkiApprovalPayeeID,b.intOrderID asc", mycon)

					Case Is = 1

						MyDataAdapter = New SqlClient.SqlDataAdapter("select a.txtApplicationCode,txtPIN,	replace(txtFullName,'|','') txtFullName,	txtEmployerName, dteApplicationDate,(select dteValueDate from tblApplicationApprovalPayee where txtApplicationCode = a.txtApplicationCode ) as ValueDate,isnull(b.txtstatus,'P') as txtstatus,b.numApproved, isnull(b.numPayingAmount,0) numPayingAmount,isnull(c.[numApprovedAmount],c.[enbloc-payment]) ApprovedAmount,	isnull(c.numAmountToPay,c.[enbloc-payment]) AmountToPay,a.IsControlChecked, a.txtControlCheckedBy from dbo.tblMemberApplication a, tblApplicationApprovalPayee b,awbr400 c where a.txtApplicationCode = b.txtApplicationCode and a.txtApplicationCode = c.txtApplicationCode and isnull(b.txtstatus,'P') = 'F' and a.txtPencomBatch = @batchNo order by b.pkiApprovalPayeeID,b.intOrderID asc", mycon)
					...
					Case Is = 6
						...
						MyDataAdapter = New SqlClient.SqlDataAdapter("select a.txtApplicationCode,txtPIN,	replace(txtFullName,'|','') txtFullName,	txtEmployerName, dteApplicationDate,(select dteValueDate from tblApplicationApprovalPayee where txtApplicationCode = a.txtApplicationCode ) as ValueDate,isnull(b.txtstatus,'P') as txtstatus,b.numApproved, isnull(b.numPayingAmount,0) numPayingAmount,isnull(c.[numApprovedAmount],c.[amount-requested-under-nsitf-from-rsa]) ApprovedAmount,	isnull(c.numAmountToPay,c.[amount-requested-under-nsitf-from-rsa]) AmountToPay,a.IsControlChecked, a.txtControlCheckedBy from dbo.tblMemberApplication a, tblApplicationApprovalPayee b,awbr200 c where a.txtApplicationCode = b.txtApplicationCode and a.txtApplicationCode = c.txtApplicationCode and isnull(b.txtstatus,'P') = 'F' and a.txtPencomBatch = @batchNo order by b.pkiApprovalPayeeID,b.intOrderID asc", mycon)

					Case Is = 5

						MyDataAdapter = New SqlClient.SqlDataAdapter("select a.txtApplicationCode,txtPIN,	replace(txtFullName,'|','') txtFullName,	txtEmployerName, dteApplicationDate,(select dteValueDate from tblApplicationApprovalPayee where txtApplicationCode = a.txtApplicationCode ) as ValueDate,isnull(b.txtstatus,'P') as txtstatus,b.numApproved, isnull(b.numPayingAmount,0) numPayingAmount,isnull(c.[numApprovedAmount],c.[total-rsa-balance]) ApprovedAmount,	isnull(c.numAmountToPay,c.[total-rsa-balance]) AmountToPay,a.IsControlChecked, a.txtControlCheckedBy from dbo.tblMemberApplication a, tblApplicationApprovalPayee b,awbr600 c where a.txtApplicationCode = b.txtApplicationCode and a.txtApplicationCode = c.txtApplicationCode and isnull(b.txtstatus,'P') = 'F' and a.txtPencomBatch = @batchNo order by b.pkiApprovalPayeeID,b.intOrderID asc ", mycon)

					Case Is = 7

						MyDataAdapter = New SqlClient.SqlDataAdapter("select a.txtApplicationCode,txtPIN,	replace(txtFullName,'|','') txtFullName,	txtEmployerName, dteApplicationDate,(select dteValueDate from tblApplicationApprovalPayee where txtApplicationCode = a.txtApplicationCode ) as ValueDate,isnull(b.txtstatus,'P') as txtstatus,b.numApproved, isnull(b.numPayingAmount,0) numPayingAmount,isnull(c.[numApprovedAmount],c.[amount-payable-net-tax]) ApprovedAmount,	isnull(c.numAmountToPay,c.[amount-payable-net-tax]) AmountToPay,a.IsControlChecked, a.txtControlCheckedBy from dbo.tblMemberApplication a, tblApplicationApprovalPayee b,awbr800 c where a.txtApplicationCode = b.txtApplicationCode and a.txtApplicationCode = c.txtApplicationCode and isnull(b.txtstatus,'P') = 'F' and a.txtPencomBatch = @batchNo order by b.pkiApprovalPayeeID,b.intOrderID asc", mycon)
					...
					Case Else
						...
						MyDataAdapter = New SqlClient.SqlDataAdapter("select a.txtApplicationCode,txtPIN,	replace(txtFullName,'|','') txtFullName,	txtEmployerName, dteApplicationDate,(select dteValueDate from tblApplicationApprovalPayee where txtApplicationCode = a.txtApplicationCode ) as ValueDate,isnull(b.txtstatus,'P') as txtstatus,b.numApproved, isnull(b.numPayingAmount,0) numPayingAmount,a.IsControlChecked, a.txtControlCheckedBy from dbo.tblMemberApplication a, tblApplicationApprovalPayee b where a.txtApplicationCode = b.txtApplicationCode and isnull(b.txtstatus,'P') = 'F' and a.txtPencomBatch = @batchNo order by b.pkiApprovalPayeeID,b.intOrderID asc", mycon)
			...
			ElseIf rtype = "C" Then
				...
				Select Case typeID
					...
					Case Is = 17
						...
						MyDataAdapter = New SqlClient.SqlDataAdapter("select a.txtApplicationCode,txtPIN,	replace(txtFullName,'|','') txtFullName,	txtEmployerName, dteApplicationDate,(select dteValueDate from tblApplicationApprovalPayee where txtApplicationCode = a.txtApplicationCode ) as ValueDate,isnull(b.txtstatus,'P') as txtstatus,b.numApproved AmountToPay, a.IsControlChecked, a.txtControlCheckedBy from dbo.tblMemberApplication a, tblApplicationApprovalPayee b where a.txtApplicationCode = b.txtApplicationCode and isnull(b.txtstatus,'P') = 'C' and a.txtPencomBatch = @batchNo order by b.pkiApprovalPayeeID,b.intOrderID asc", mycon)
	...
	Public Sub PMInsertRMASScheduleHardShip(lstRMASSchedule As List(Of RMASSchedule), SPLogBatchNo As String, apptype As String, submissionType As Integer, uName As String)
		...
		Try
			...
			Do While i < lstRMASSchedule.Count
				...
				ElseIf submissionType = 1 And apptype = "17" Then
