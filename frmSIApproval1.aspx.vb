...
Partial Class frmSIApproval1
	...
	Protected Sub generateSI()
		...
		If ddRunType.Text = "Renewal" Then
			...
			dt = cr.PMgetNewStandingOrder("R", "CC", intAnnMonth, CInt(ddAnnYear.Text))
			ViewState("Renewal") = dt
		Else

			dt = cr.PMgetNewStandingOrder("N", "CC", intAnnMonth, CInt(ddAnnYear.Text))
			ViewState("New") = dt
		End If
			...
			If IsNothing(ViewState("TagAll")) = True Then
				...
				For Each grow As GridViewRow In Me.gridStandingOrder.Rows
					...
					If cb.Checked = True Then
						cr.PMUpdateStandingOrderApproval(CInt(grow.Cells(1).Text), Session("user"), "A")
					End If
				...
				If IsNothing(ViewState("New")) = False And IsNothing(ViewState("TagAll")) = False Then
					...
					If i = 1 Then
						...
						Do While i < dt.Rows.Count
							cr.PMUpdateStandingOrderApproval(dt.Rows(i).Item("pkiSIApproval"), Session("user"), "A")
							i = i + 1
						Loop
