...
Partial Class frmPensionEnhancementCheckBPD
    ...
    Protected Sub btnFind_Click(sender As Object, e As EventArgs) Handles btnFind.Click
		...
		dt = cr.PMgetPencomEnhancementMultiplePIN(CDate(txtRunDate.Text), Server.MapPath("~/Logs"), "BPD", str)
    ...
    Public Sub PMUpdateApprovalControlCheck(pin As String, runDate As Date, uName As String)
        Try
			...
			myComm.CommandText = "update tblPensionEnhancement set  isBPDConfirmed = 1 ,txtBPDConfirmedBy = '" & uName & "', dteBPDConfirmed = '" & DateTime.Parse(Now).ToString("yyyy-MM-dd HH:MM") & "' where dteRunFor = '" & DateTime.Parse(runDate).ToString("yyyy-MM-dd") & "' and txtPIN = '" & pin & "'  "
            ...
            dt = cr.PMgetPencomEnhancement(CDate(txtRunDate.Text), Server.MapPath("~/Logs"), "BPD")
            BindGrid(dt)
	...
	Protected Sub gridApplications_PageIndexChanging(sender As Object, e As GridViewPageEventArgs) Handles gridApplications.PageIndexChanging
		...
		If IsNothing(ViewState("dtEnhancement")) = False Then
